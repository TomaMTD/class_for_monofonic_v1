...
ClassEngine::ClassEngine(const ClassParams &pars, bool verbose) : cl(0), dofree(true)
{
  ...
  {
    if (fc.read[i] != _TRUE_)
      throw invalid_argument(string("invalid CLASS parameter: ") + fc.name[i]);
  }
...
ClassEngine::ClassEngine(const ClassParams &pars, const string &precision_file, bool verbose) : cl(0), dofree(true)
{
  ...
  // concatenate both
  if (parser_cat(&fc_input, &fc_precision, &fc, _errmsg) == _FAILURE_)
    throw invalid_argument(_errmsg);
...
void ClassEngine::call_perturbations_sources_at_tau(
    int index_md,
    int index_ic,
    int index_tp,
    double tau,
                        ...
                        double &fHa)
{
  ...
  if (!dofree)
    throw out_of_range("no Tk available because CLASS failed");
...
ClassEngine::getCl(Engine::cltype t, const long &l)
{
  ...
  if (!dofree)
    throw out_of_range("no Cl available because CLASS failed");
